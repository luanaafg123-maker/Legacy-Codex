    ...
    <div>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-muted text-sm mt-1 font-mono">Reposição, troca e peças técnicas</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 opacity-60 pointer-events-none select-none">
        {[1, 2, 3].map((i) => (
          <div key={i} className="card-hover h-48 flex items-center justify-center bg-surface/30">
            <span className="text-muted font-mono text-sm">Listagem {i}</span>
          </div>
        ))}
      </div>
      <p className="text-center text-muted text-xs mt-8 italic">Marketplace em desenvolvimento. Dados serão populados via Supabase.</p>
    </div>
  );
}
